...
public class EnemyController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player = GameObject.FindGameObjectWithTag("Player").transform; //Player Tag를 가지고 있는 오브젝트 transform 을 입력
    }
    ...
    void Update()
    {
        if(player != null)
        {
            ...
            {
                Vector3 direction = (player.position - transform.position).normalized;      //이동
